...
engineering trace data, weaker techniques, \eg quasi-experiments, are employed.
...
time $t$ from the start of the observation period;
\emph{intervention} is an indicator for time $t$ occurring before 
...
of months after the intervention at time $t$, coded 0 before CI adoption and 
...
A critical assumption for RDD is that but for the treatment, all other variables 
...
\begin{figure}[t]
	...
	\caption{RDD illustration.\vf{Replace this with PDF version and bigger fonts}}\vspace{-0.5cm}
	\label{RDDIllustration}
\end{figure}

We wish to capture, using the RDD model above, changes in trends after CI 
adoption across our sample of projects, while controling for confounding variables 
(\eg project size, age, and programming language; see Measures subsection above).
Our data is centered at the time of CI adoption, and has an equal number of points, 
12, on each side.
Since our data is inherently nested (each project contributes multiple observations; 
