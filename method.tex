...
The kind of activity being performed is an important confound for commit size,
since one can reasonably expect, \eg that bug fixes are smaller than new feature
implementations.
...
Note that we don't enforce that commits messages that match our regular
...
use \GH's issue tracker) and project linking / commit message conventions.
%
...
In addition to these data described above, we parsed the \Tvis build logs and 
...
of the simpler sharp RDD approaches, namely linear regression with an interaction term.
