...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth, clip=true, trim=0 0 0 32]{figures/freq-non-merge.pdf}
\includegraphics[width=0.9\columnwidth, clip=true, trim=0 0 0 0]{figures/freq-merge.pdf}
\caption{Commit frequency before and after the \Tvis adoption.}\vspace{-0.4cm}
\label{fig:freq}
\end{figure}


\subsection{RQ$_1$: Trends in Commit Frequency}
...
We modeled a random intercept for \emph{programming language}, 
...
we also modeled a random \emph{intervention} slope and intercept for \emph{project}, to allow 
...
\subsection{RQ$_2$: Trends in Code Churn}
...
%\Tvis is applied solely to pull requests, \ie 
...
\subsection{RQ$_3$: Trends in Pull Request Closing}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth, clip=true, trim=0 5 0 0]{figures/pr-latency.pdf}
\caption{Mean pull request latency before and after the \Tvis adoption.}\vspace{-0.3cm}
...
\Tvi quality gate, and can then rapidly react %to the feedback provided by \Tvi 
by correcting the pull request source code.
Hence, we expect that \Tvis shortens the pull request latency and
%under the assumption of constant productivity, 
...
%, for each of twelve consecutive 30-day time intervals before, and after, the \Tvis adoption. 
%Similarly to Figure~\ref{fig:churn}, 
...
Moreover, surprisingly, we find that the initial trend is flattened post-\Tvi,
...
This, again, is quite surprising, as we have seen in RQ$_2$ above that the size of 
code changes becomes, on average, smaller %and smaller 
...
One possible explanation for the increased latency is the %low performance 
\Tvi' slowness, reported by some survey participants (R1, R27, R53).


\subsection{RQ$_4$: Trends in Issue Closing}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth, clip=true, trim=0 5 0 0]{figures/issues-closed.pdf}
\caption{Number of closed issues before and after the \Tvis adoption.}
\label{fig:issues}\vspace{-0.3cm}
\end{figure}
...
\subsection{RQ$_5$: Trends in Testing}
...
%\footnote{The median number of error types per build remained $2$ over 
%time.}
