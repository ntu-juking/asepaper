...
\smallskip\noindent \as{Added here} \emph{Survey}:
The expected decrease in the size of the commits is echoed by one of the survey respondents: ``commits became smaller and more frequently, to check the build, pull request became easier to check'' (R4).
At the same time, a different respondent indicated that \Tvis discourages them from the trivial commits (R11), suggesting that the commits they perform are less trivial and likely to be larger\as{too speculative?}.
Furthermore, several developers referred to pull request integration as the reason for introduction of \Tvis,
and while R31 and R50 state that \Tvis is used both for commits and for pull requests, R11, R21 and R37 explicitly state that \Tvis is applied solely to pull requests, i.e., in those projects direct commits are outside the scope of \Tvis.
